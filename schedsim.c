...
{ 
  ...
  int time_left[n];
  for (int i = 0; i < n; i++){
    time_left[i] = plist[i].bt;
  }
  int current_time = 0;
  int completed = 0;
  while(completed!=n){
    for (int i= 0; i < n; i++){
        if(time_left[i]>quantum){
...
{ 
  ...
  while (completed != n){
    ...
    for (int i = current_process+1; i <n ; i++){
      if (current_time >= plist[i].art){
        if (time_left[i]!=0 && time_left[i] < time_left[current_process]){
          current_process = i;
        }
      }
    }
    if (current_time >=plist[0].art){
      time_left[current_process]--;
      current_time++;
      if (time_left[current_process] == 0){
        completed++;
        plist[current_process].tat = current_time;
        plist[current_process].wt = current_time - plist[current_process].art - plist[current_process].bt;
      }
    }
    else{
      current_time++;
...
{ 
    ...
    */
    ...
    qsort(plist, n, sizeof(*plist), my_comparer);
